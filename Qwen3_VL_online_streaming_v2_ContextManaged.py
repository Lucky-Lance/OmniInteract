...
# Monotonically-increasing generation epoch. Bumped every time the user
# interrupts (Type 11). Sentences are stamped with the epoch active at
# enqueue time; the TTS worker drops any sentence whose epoch is older
# than the current one, so everything queued *before* an interrupt is
# silently discarded without needing to know its response_id.
tts_epoch: int = 0
...
def tts_worker_loop(args):
    ...
    while True:
        ...
        try:
            ...
            # If this sentence was queued BEFORE the most recent user
            # interrupt, silently drop it. This closes the window where a
            # worker popped a stale task right before Type 11 arrived.
            # Do NOT clear the cancel flag in that branch — the in-flight
            # sentence still relies on it to break out of its streaming loop.
            task_epoch = task.get("epoch", 0)
            with pending_tts_lock:
                if task_epoch < tts_epoch:
                    print(f"⏹ [TTS] Dropping stale sentence {sentence_idx} "
                          f"(epoch {task_epoch} < {tts_epoch})")
                    continue
...
async def handle_client_connection_async(conn, addr, args):
    ...
    try:
        while True:
            ...
            elif file_type == 11:  # User speech started (VAD interrupt)
                # Hard interrupt: the client's VAD detected that the user
                # started speaking, so we immediately stop the current
                # generation *and* any TTS work that was in flight. The
                # context (history / video frames) is preserved so the
                # upcoming ASR query can be answered with full context.
                print("🎙 [Interrupt] User speech started — cancelling "
                      "generation + TTS")
                if session.current_task and not session.current_task.done():
                    session.current_task.cancel()
                session.is_generating = False
                session.is_auto_generating = False
                # Drop any sentences still waiting in the TTS queue.
                clear_tts_sentence_queue()
                # Bump the generation epoch + raise the cancel flag so
                #   (a) the currently-streaming TTS sentence breaks out of
                #       its audio-chunk loop via should_cancel_tts(), and
                #   (b) any sentence already popped by the worker but not
                #       yet processed gets dropped on the epoch check.
                global tts_cancel_flag, tts_epoch
                with pending_tts_lock:
                    tts_cancel_flag = True
                    tts_epoch += 1
                    print(f"   ↳ bumped tts_epoch={tts_epoch}")
                # Forget any stale prompt so the next video-trigger does
                # nothing before the fresh ASR result arrives.
                last_prompt = ""
