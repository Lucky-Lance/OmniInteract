...
async def stream_tts(req: TTSRequest, request: Request):
    ...
    # --- Phase 1: acquire tts_lock asynchronously ---
    # The old code did `with tts_lock` inside the sync generator _iter(),
    # which blocked a threadpool thread.  While blocked, the async
    # _watch_disconnect() could (falsely) fire, setting cancel_event
    # before any data was ever yielded — leaving the client hanging.
    while not tts_lock.acquire(blocking=False):
        if await request.is_disconnected():
            print("🔌 [TTS] Client disconnected while waiting for lock")
            return StreamingResponse(iter(()), media_type="application/octet-stream")
        await asyncio.sleep(0.05)

    # --- Phase 2: generate & stream (lock already held) ---
    cancel_event = threading.Event()

    async def _watch_disconnect():
        while not cancel_event.is_set():
            if await request.is_disconnected():
                # Confirm after a short delay to avoid transient false positives
                await asyncio.sleep(0.5)
                if cancel_event.is_set():
                    return
                if await request.is_disconnected():
                    cancel_event.set()
                    print("🔌 [TTS] Client disconnected, cancelling generation")
                    return
    ...
    def _iter():
        try:
            for pcm_bytes, sr in _generate_pcm(
                ...
                if cancel_event.is_set():
                    print("⏹ [TTS] Generation aborted (client gone)")
                    break
    ...
    try:
        ...
        response = StreamingResponse(_iter(), media_type="application/octet-stream")
