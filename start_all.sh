...
echo "🧹 Checking for leftover processes on ports 8001, 8002, 12346..."
kill_port 8001
kill_port 8002
kill_port 12346
...
# ── 3. 主推理服务 (port 12345) ──
echo "🚀 Starting vLLM inference server (GPU $GPU_INFERENCE, TP=$TP_SIZE, port 12346)..."
...
# 等待 vLLM listen 端口 12345（模型加载可能较久，这里给 10 分钟）
echo "    Waiting for vLLM server (port 12346) to be ready..."
VLLM_READY=0
for i in $(seq 1 300); do
    if ss -tln 2>/dev/null | grep -q ':12346 '; then
        echo "    ✓ vLLM server ready"
        VLLM_READY=1
        break
    fi
    if ! kill -0 "${PIDS[-1]}" 2>/dev/null; then
        echo "    ✗ vLLM process exited unexpectedly, check logs/vllm.log"
        cleanup
    fi
    sleep 2
done

# ── 4. 端到端预热 (ASR → TTS-多长度 → vLLM E2E) ──
if [ "$VLLM_READY" = "1" ]; then
    echo "🔥 Running end-to-end warmup (logs/warmup.log)..."
    if python -u warmup.py > "$LOG_DIR/warmup.log" 2>&1; then
        echo "    ✓ warmup ok (logs/warmup.log)"
    else
        echo "    ⚠️  warmup returned non-zero (services still usable, see logs/warmup.log)"
    fi
else
    echo "    ⚠️  skipping warmup: vLLM not ready in time"
fi
...
echo "  vLLM: port 12346            (GPU $GPU_INFERENCE, TP=$TP_SIZE)"
echo ""
echo "  Logs: $LOG_DIR/"
